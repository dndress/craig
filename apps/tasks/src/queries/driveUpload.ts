...
async function findCraigDirectoryInGoogleDrive(drive: drive_v3.Drive) {
  try {
    const list = await drive.files.list({
      q: "name = 'Chronicler' and mimeType = 'application/vnd.google-apps.folder'"
    });
...
function getRecordingDescription(recordingId: string, info: any, joiner = '\n') {
  return [
    `Chronicler recording ${recordingId}`,
    '',
      ...
      case 'google': {
        ...
        const file = await drive.files.create({
          ...
          requestBody: {
            ...
            contentHints: {
              indexableText: `${info.channel} - ${info.guild} - Chronicler recording ${recordingId}`
            }
          },
          media: {
            mimeType: mime,
